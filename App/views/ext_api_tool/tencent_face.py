...
# 一.计算接口鉴权，构造请求参数
from App.settings import FONTLIB_DIR, FACE_DIR, TENCENT_APP_ID, TENCENT_APP_KEY
...
def get_params(img):
    ...
    sort_dict.append(('app_key', TENCENT_APP_KEY))  # 添加app_key
    ...
    if res['ret'] == 0:  # 0代表请求成功
        ...
        for obj in res['data']['face_list']:
            ...
            if len(res['data']['face_list']) > 1:  # 检测到多个人脸，就把信息写入人脸框内
                font = ImageFont.truetype(FONTLIB_DIR+'/yahei.ttf', w // 8, encoding='utf-8')  # 提前把字体文件下载好
                ...
                font = ImageFont.truetype(FONTLIB_DIR+'/yahei.ttf', w // 8, encoding='utf-8')
                draw.text((x + 10, y + 10), '性别 :' + gender, (76, 176, 80), font=font)
                draw.text((x + 10, y + 10 + delt * 1), '年龄 :' + str(obj['age']), (76, 176, 80), font=font)
                draw.text((x + 10, y + 10 + delt * 2), '表情 :' + expression, (76, 176, 80), font=font)
                draw.text((x + 10, y + 10 + delt * 3), '魅力 :' + str(obj['beauty']), (76, 176, 80), font=font)
                ...
                font = ImageFont.truetype(FONTLIB_DIR+'/yahei.ttf', 20, encoding='utf-8')
            ...
            cv2.imwrite('{}/{}'.format(FACE_DIR,os.path.basename(img)), cv2img)  # 保存图片到 face 文件夹下
        return 'success'
    else:
        return 'fail'